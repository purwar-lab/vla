...
- **target_position:** Cuboid position is randomly selected from 3 fixed positions, and then randomized about that point so that the cuboids are not in a straight line. The cuboid yaw is randomized as well. The cardboard box is placed behind the target cuboid, with some randomness.
