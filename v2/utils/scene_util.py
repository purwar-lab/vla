...
GRIPPER_BUFFER = 0.06
GRIPPER_ACTIONS = {"close": 0, "open": 1}
...
class CustomSceneCfg(InteractiveSceneCfg):
    ...
    kitchen = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(
            ...
            rot=quat_from_euler_xyz(
                torch.tensor(0), torch.tensor(0), torch.tensor(math.radians(-45))
            ),
        ),
    )

    # articulation
    robot = trossen_robots.get_robot_articulation_cfg(
        robot_name=ROBOT_NAME, prim_path="{ENV_REGEX_NS}/Robot", pos=(0, 0, 0.88)
    )

    # camera_wrist_right
    camera_wrist_right = cameras.get_camera_cfg(
        prim_path="{ENV_REGEX_NS}/Robot/"
        + ROBOT_NAME
        + "_gripper_bar_link/camera_wrist_right",
        resolution="300x300",
        focal_length=10,
        pos=(-0.035, 0.01, 0.1),
        rot=(0, 20, 0),
    ...
    # camera_high
    camera_high = cameras.get_camera_cfg(
        prim_path="{ENV_REGEX_NS}/Robot/camera_high",
        resolution="300x300",
        pos=(1.4, 0, 0.65),
        rot=(0, 30, -180),
    )

    red_cuboid = custom_items.get_item_config(
        item_name="red_cuboid", mass=0.05, pos=(20, 4, 0.06)
    )
    green_cuboid = custom_items.get_item_config(
        item_name="green_cuboid", mass=0.05, pos=(20, 10, 0.06)
    )
    blue_cuboid = custom_items.get_item_config(
        item_name="blue_cuboid", mass=0.05, pos=(20, 12, 0.06)
    )
...
class CustomScene:
    def __init__(self, args_cli):
        ...
        self.sim.set_camera_view(
            PERSPECTIVE_CAMERA_POSITION, PERSPECTIVE_CAMERA_LOOK_AT
        )
        # Design scene
        scene_cfg = CustomSceneCfg(
            num_envs=1, env_spacing=5.0
        )  # Hardcoding it to use only 1 env. Multi envs have some weird issue that I don't need to resolve right now
        ...
        self.gripper_pos_limits = (
            self.robot.data.soft_joint_pos_limits[0, 7].clone().cpu().numpy().tolist()
        )
        ...
        self.robot_entity_cfg = SceneEntityCfg(
            ...
            body_names=[f"{ROBOT_NAME}_fingers_link"],
        )
    ...
    def place_items_randomly(self, episode_index=0, capture_data_flag=False):
        ...
        for i in range(len(all_item_names)):
            ...
            x_y_z_roll_pitch_yaw = [
                x,
                y,
                z,
                0,
                0,
                random.uniform(math.radians(-45), math.radians(45)),
            ]
        ...
        joint_pos[0, :6] = torch.tensor(
            trossen_robots.get_robot_starting_pos(ROBOT_NAME)
        )
        ...
        distance = custom_util.get_distance_between_two_points(
            [x, y, z], [look_x, look_y, look_z]
        )
        ...
        initial_x_y_z_roll_pitch_yaw_gripper = [
            x,
            y,
            z,
            roll,
            pitch,
            yaw,
            GRIPPER_ACTIONS["open"],
        ]

        self.move_ee_to_pos(
            initial_x_y_z_roll_pitch_yaw_gripper, max_steps=500, world=True
        )
        ...
        for _ in range(
            2
        ):  # Multiple steps as workaround to update the camera and other sensors and prevent stale data
            self.update_scene()
        ...
        self.scene_info["initial_x_y_z_roll_pitch_yaw_gripper"] = (
            initial_x_y_z_roll_pitch_yaw_gripper
        )
        ...
        self.scene_info["grab_x_y_z_roll_pitch_yaw_gripper"] = [
            grab_x,
            grab_y,
            grab_z,
            grab_roll,
        ...
        self.scene_info["place_x_y_z_roll_pitch_yaw_gripper"] = [
            place_x,
            place_y,
            place_z,
            place_roll,
    ...
    def place_items_from_hdf5_file(self, hdf5_file_path, capture_data_flag=False):
        ...
        with h5py.File(hdf5_file_path, "r") as hdf5_fobj:
            ...
            for item_name in ITEM_NAMES:
                ...
                self.place_item_at_x_y_z_roll_pitch_yaw(
                    item_name, x_y_z + roll_pitch_yaw
                )
    ...
    def get_ee_pos_and_quat_with_respect_to_robot(self):
        ...
        return self.convert_from_world_frame_to_robot_frame(
            ee_pose_w[:, 0:3], ee_pose_w[:, 3:7]
        )

    def normalize_gripper_action(self, gripper_action):
        normalized_gripper_action = (gripper_action - self.gripper_pos_limits[0]) / (
            self.gripper_pos_limits[1] - self.gripper_pos_limits[0]
        )
        return normalized_gripper_action

    def unnormalize_gripper_action(self, normalized_gripper_action):
        gripper_action = self.gripper_pos_limits[0] + (
    ...
    def calculate_joint_pos_from_ik(self):
        ...
        temp_target_joint_pos = self.diff_ik_controller.compute(
            ee_pos_b, ee_quat_b, jacobian, joint_pos[:, self.robot_entity_cfg.joint_ids]
        )
    ...
    def move_ee_to_pos(
        self,
        x_y_z_roll_pitch_yaw_gripper,
        max_steps=MAX_SUB_STEPS,
        world=True,
        capture_data_flag=False,
    ):
        ...
        quat[:] = quat_from_euler_xyz(
            torch.tensor(x_y_z_roll_pitch_yaw_gripper[3]),
            torch.tensor(x_y_z_roll_pitch_yaw_gripper[4]),
            torch.tensor(x_y_z_roll_pitch_yaw_gripper[5]),
        )
        if world:
            relative_x_y_z, relative_quat = (
                self.convert_from_world_frame_to_robot_frame(x_y_z, quat)
            )
        else:
            relative_x_y_z = x_y_z
            relative_quat = quat
        self.ik_target[0, :] = torch.tensor(
            [*relative_x_y_z[0], *relative_quat[0]], device=self.sim.device
        )
        ...
        for step_index in range(1, max_steps + 1):
            ...
            if capture_data_flag and (
                ik_loss < IK_LOSS_THRESHOLD
                or step_index % CAPTURE_DATA_STEP_INTERVAL == 0
            ):
                self.capture_data()
    ...
    def apply_qpos_to_robot(
        self, qpos, max_steps=MAX_SUB_STEPS, capture_data_flag=False
    ):
        # with open("singleenv_a.log", "a") as fp:
        #     fp.write(f"{qpos}\n")
        ...
        # with open("singleenv_b.log", "a") as fp:
        #     fp.write(f"{self.target_joint_pos.numpy(force=True)}\n")
        ...
        # with open("singleenv_c.log", "a") as fp:
        # fp.write(f"{self.robot.data.joint_pos.clone().numpy(force=True)}\n")
    ...
    def get_current_x_y_z_roll_pitch_yaw_of_item(self, item_name):
        ...
        x_y_z_roll_pitch_yaw[3:] = [
            custom_util.convert_reflex_angle_to_negative_angle(x[0].item())
            for x in item_roll_pitch_yaw
        ]
    ...
    def place_item_at_x_y_z_roll_pitch_yaw(self, item_name, x_y_z_roll_pitch_yaw):
        ...
        item_state[:, 3:7] = quat_from_euler_xyz(
            torch.tensor(x_y_z_roll_pitch_yaw[3]),
            torch.tensor(x_y_z_roll_pitch_yaw[4]),
            torch.tensor(x_y_z_roll_pitch_yaw[5]),
        )
    ...
    def get_current_observation(self):
        ...
        for i in range(3):
            roll_pitch_yaw[i] = custom_util.convert_reflex_angle_to_negative_angle(
                roll_pitch_yaw[i][0].cpu().item()
            )
        ...
        x_y_z_roll_pitch_yaw_gripper = [
            *ee_pos_b[0].clone().cpu().numpy(),
            *roll_pitch_yaw,
            normalized_gripper_state,
        ]

        camera_wrist_right_data_tensor = camera_wrist_right.data.output["rgb"]
        camera_wrist_right_data_tensor = camera_wrist_right_data_tensor.view(
            camera_wrist_right_data_tensor.shape[1:]
        )
        camera_wrist_right_data = camera_wrist_right_data_tensor.cpu().numpy()

        # camera_high
        camera_high_data_tensor = camera_high.data.output["rgb"]
        camera_high_data_tensor = camera_high_data_tensor.view(
            camera_high_data_tensor.shape[1:]
        )
        ...
        # with open("singleenv_d.log", "a") as fp:
        #     fp.write(f"{json.dumps(observation, default=lambda x: str(x))}\n")
        return observation

    def capture_data(self):
        observation = self.get_current_observation()
        self.captured_data["/observations/x_y_z_roll_pitch_yaw_gripper"].append(
            observation["x_y_z_roll_pitch_yaw_gripper"]
        )
        self.captured_data["/observations/qpos"].append(observation["qpos"])
        self.captured_data["/observations/images/camera_high"].append(
            observation["camera_high_data"]
        )
        self.captured_data["/observations/images/camera_wrist_right"].append(
            observation["camera_wrist_right_data"]
        )

    def create_hdf5_from_captured_data(
        self, save_directory, episode_index, sub_episode_last_step_index_list
    ):
        ...
        for cam_name in camera_names:
            compressed_image_list = self.captured_data[
            ...
            self.captured_data[f"/observations/images/{cam_name}"] = (
                padded_compressed_image_list
            )
        ...
        with h5py.File(dataset_path, "w", rdcc_nbytes=1024**2 * 2) as root:
            ...
            for item_name in self.scene_info["initial_item_positions"].keys():
                root.attrs[f"{item_name}_position"] = self.scene_info[
                    "initial_item_positions"
                ][item_name]

            for item_name in self.scene_info["initial_item_orientations"].keys():
                root.attrs[f"{item_name}_orientation"] = self.scene_info[
                    "initial_item_orientations"
                ][item_name]
            root.attrs["sub_episode_last_step_index_list"] = (
