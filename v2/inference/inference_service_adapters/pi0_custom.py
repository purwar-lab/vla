...
class InferenceService:
    ...
    def predict(self, current_observation):
        ...
        data = {
            "observation.images.image": cv2.resize(current_observation["camera_high_data"], (224,224), interpolation=cv2.INTER_AREA),
            "observation.images.ego_image": cv2.resize(current_observation["camera_wrist_right_data"], (224,224), interpolation=cv2.INTER_AREA),
            "state": np.array(current_observation["qpos"], dtype=np.float32),
            "task": current_observation["task"]
        }
        
        actions = self.pi_zero_client.infer(data)["actions"].tolist()
